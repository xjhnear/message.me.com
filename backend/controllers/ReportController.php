...
class ReportController extends BaseController
{
    ...
    public function actionAdd()
    {
        ...
        if (Yii::$app->request->isPost) {
            ...
            $data = Yii::$app->request->post('Report');
    ...
    public function actionEdit()
    {
        ...
        if (Yii::$app->request->isPost) {
            $data = Yii::$app->request->post('Report');//var_dump($data);exit();
    ...
    {
        /*$data = Report::find()->where(['pay_status' => 1])
            ->andWhere(['status'=>1])
            ->orderBy('report_id DESC')
            ->asArray()->all();*/
        $arr = [];
        $title = ['report_id', '订单号', '用户ID', '姓名', '电话', '身份证', '商品类型', '套餐ID', '商品ID', '商品名', '起租时间', '退租时间',
