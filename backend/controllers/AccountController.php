...
class AccountController extends BaseController
{
    ...
    public function actionEdit()
    {
        ...
        if (Yii::$app->request->isPost) {
            $data = Yii::$app->request->post('Account');//var_dump($data);exit();
    ...
    {
        /*$data = Account::find()->where(['pay_status' => 1])
            ->andWhere(['status'=>1])
            ->orderBy('account_id DESC')
            ->asArray()->all();*/
        $arr = [];
        $title = ['account_id', '订单号', '用户ID', '姓名', '电话', '身份证', '商品类型', '套餐ID', '商品ID', '商品名', '起租时间', '退租时间',
